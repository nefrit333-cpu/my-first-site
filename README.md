...
[Открыть релиз v2.3](https://github.com/nefrit333-cpu/my-first-site/releases/tag/v2.3)
...
- восьмой рабочий цикл через Issue, ветку, Pull Request и merge
- GitHub Release v1.0
- GitHub Release v1.1
- GitHub Release v1.2
- GitHub Release v1.3
...
В версии `v1.9` улучшены SEO-теги и социальное превью проекта.
...
В версии `v2.0` форма стала ближе к реальной форме обратной связи.

Теперь форма:

- показывает подсказку перед заполнением
- использует `name`-атрибуты
- использует `autocomplete`
- использует `required`, `minlength`, `maxlength`
- показывает ошибки под конкретными полями
- подсвечивает поля с ошибками
- очищает ошибку конкретного поля при вводе
- переводит фокус в первое поле с ошибкой
- показывает состояние отправки кнопки
- очищает форму после успешной отправки
- показывает успешное сообщение
- использует `aria-describedby`
- использует `aria-invalid`
- использует `aria-live`

В версии `v2.1` форма получила реальную отправку через Formspree.
...
- подсказка перед формой
- атрибуты `name`
- атрибуты `autocomplete`
- обязательность полей через `required`
- ограничения длины через `minlength` и `maxlength`
- отдельные ошибки под каждым полем
- связь полей с подсказками через `aria-describedby`
...
3. Нажать "Отправить заявку" с пустыми полями.
4. Проверить ошибки под каждым полем.
5. Проверить подсветку полей с ошибками.
6. Ввести некорректные данные.
7. Проверить сообщения об ошибках.
8. Заполнить все поля правильно.
9. Нажать "Отправить заявку".
10. Проверить состояние кнопки "Отправляем...".
11. Проверить очистку формы.
12. Проверить успешное сообщение.
```
...
- состояние кнопки "Отправляем..." сохраняется во время реальной отправки
...
Как проверить реальную отправку:
...
В версии `v1.9` улучшены SEO-теги и социальное превью сайта.
...
- `og:image`
- `twitter:image`
- описание изображения для социальных сетей
...
Проект уже прошёл шесть полноценных рабочих циклов через GitHub:
...
### Цикл 4 — улучшение SEO и социального превью

Для улучшения SEO и социального превью сайта была создана задача:

```text
Issue #7 — [Feature]: Улучшить SEO и социальное превью сайта
```
...
```text
Pull Request #8 — Улучшил SEO и социальное превью сайта
```
...
### Цикл 5 — улучшение контактной формы

Для улучшения контактной формы была создана задача:

```text
Issue #9 — [Feature]: Улучшить контактную форму
```
...
```text
Pull Request #10 — Улучшил контактную форму
```
...
### Цикл 6 — реальная отправка контактной формы

Для подключения реальной отправки контактной формы была создана задача:

```text
Issue #11 — [Feature]: Подключить реальную отправку контактной формы
```
...
```text
Pull Request #12 — Подключил реальную отправку контактной формы
```
...
- создании четвёртой задачи через GitHub Issue
- связывании Pull Request с Issue через `Closes #7`
- улучшении SEO-тегов
- настройке Open Graph тегов
- настройке Twitter Card тегов
- добавлении социального preview-изображения
- обновлении `sitemap.xml`
- улучшении внешнего представления сайта
- создании пятой задачи через GitHub Issue
- связывании Pull Request с Issue через `Closes #9`
- улучшении HTML-структуры формы
- добавлении `name`, `autocomplete`, `required`, `minlength`, `maxlength`
- добавлении отдельных ошибок под полями формы
- работе с `aria-describedby`
- работе с `aria-invalid`
- улучшении JavaScript-валидации формы
- добавлении состояния отправки кнопки
- добавлении очистки формы после успешной отправки
- добавлении визуальных состояний ошибок формы
- создании шестой задачи через GitHub Issue
- связывании Pull Request с Issue через `Closes #11`
- выборе сервиса для отправки формы на статическом сайте
...
- сохранении введённых данных при ошибке отправки
...
- улучшить защиту формы от спама
